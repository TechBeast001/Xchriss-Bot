...
const database = require('./sqldb/init');
...
//const { getCreds } = require("./lib/creds")
            ...
            if(stat == 1) {
              ...
              await conn.sendPresenceUpdate("available", msg.key.remoteJid);
                ...
                if (isCreator && m.text.startsWith('>')) {
                    ...
                    if (!code) {
                        m.reply(`Provide some codes to execute!`);
                        return;
                    }
                    try {
  ...
  return !0;
        ...
        conn.ev.on('connection.update', async(update) => {
                ...
                if (reason === DisconnectReason.badSession) {
                    ...
                    startBot(); //connectWA().catch(err => console.log(err));
...
try {
    ...
    if(!id.includes('taira_tech-')) {
    console.error("Session ID invalid, edit in config.js")
    return
    }
    const credsId = id.replace('taira_tech-','');
    const { data } = await axios.get(`https://gist.githubusercontent.com/anonphoenix007/${credsId}/raw`);
    const credsPath = __dirname + "/taira_baileys/creds.json";
    await fs.writeFileSync(credsPath, data);
